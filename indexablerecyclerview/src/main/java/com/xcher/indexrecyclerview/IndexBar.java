...
//        }
    ...
    void setData(ArrayList<EntityWrapper> data) {
        this.mDatas = data;
        ...
        for (int i = 0; i < data.size(); i++) {
            EntityWrapper wrapper = data.get(i);
            if (wrapper.getItemType() == EntityWrapper.TYPE_TITLE || wrapper.getIndexTitle() == null) {
                String index = wrapper.getIndex();
                if (!TextUtils.isEmpty(index)) {
                    if (IndexRecyclerView.INDEX_SIGN.equals(index)) {
                        mIndexList.add(IndexRecyclerView.INDEX_SIGN);
                    } else if (!mIndexList.contains(index)) {
                        if (wrapper.getHeaderFooterType() == EntityWrapper.TYPE_HEADER && !tempHeaderList.contains(index)) {
